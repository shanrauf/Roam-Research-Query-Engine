...
(deftest roam-native-spec []
  ...
  (testing "Roam native results"
    [(is (= (roam-native-query [] test-query-1) test-1-result))
     (is (= (roam-native-query [] test-query-2) test-2-result))
     (is (= (roam-native-query [] test-query-3) test-3-result))
     (is (= (roam-native-query [] test-query-4) test-4-result))
     (is (= (roam-native-query [] test-query-5) test-5-result))
     (is (= (roam-native-query [] test-query-6) test-6-result))
     (is (= (roam-native-query [] test-query-7) test-7-result))
     (is (= (roam-native-query [] test-query-8) test-8-result))
     (is (= (roam-native-query [] test-query-9) test-9-result))]))
